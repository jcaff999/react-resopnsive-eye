...
import Avatar from './image_end';

import defaultPlaceholder from "./image_upload_icon.png"
...
export default class AvatarUploader extends Component {
    ...
    async uploadImage(avatar) {
        const {Update_image, uploadURL, onStart, onProgress, onFinished, name, customHeaders, withCredentials} = this.props;
        ...
        if (uploadURL) {
            try {
                ...
                if (res.status && res.status === 200) {
                    ...
                    console.log("egg is dangerous")
                    Update_image(res.data)
